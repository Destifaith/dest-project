...
const handleSubmit = (e: React.FormEvent) => {
  ...
  data.append("has_daily_menu", formData.has_daily_menu ? "1" : "0");
  data.append("daily_menu_email", formData.daily_menu_email || "");
  ...
  if (formData.main_image) {
    ...
    console.log("✅ Main image appended:", formData.main_image.name);
  }

  // FIXED: Append gallery images correctly
  if (formData.gallery_images && formData.gallery_images.length > 0) {
    formData.gallery_images.forEach((file, index) => {
      ...
      console.log(`✅ Gallery image ${index} appended:`, file.name);
    });
  }
  ...
  if (formData.menu_pdf) {
    ...
    console.log("✅ Menu PDF appended:", formData.menu_pdf.name);
  }

  // Append owner image
  if (formData.owner_image) {
    data.append("owner_image", formData.owner_image);
    console.log("✅ Owner image appended:", formData.owner_image.name);
  }
  ...
  // Append award images with proper indexing
  formData.awards.forEach((award, index) => {
    if (award.image) {
      data.append(`award_images[${index}]`, award.image);
      console.log(`✅ Award image ${index} appended:`, award.image.name);
    }
  });
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          onChange={handleChange}
          placeholder="e.g. Reservations required for groups of 6+, 24-hour cancellation policy, etc."
          className="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-primary/50 focus:outline-none"
        />
      </div>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          This will be the primary image displayed for your restaurant
        </p>
      </div>
      ...
      <div>
        ...
        />
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Upload multiple images to showcase your restaurant's ambiance, food, and facilities
        </p>
      </div>

      {/* Menu PDF */}
      <div>
        <label className="block font-medium text-gray-700 dark:text-gray-200">Restaurant Menu (PDF)</label>
        <input
          type="file"
          name="menu_pdf"
          accept=".pdf"
        ...
        />
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Upload your full menu as a PDF file
        </p>
      </div>
        ...
        <div className="mb-4">
          <label className="flex items-center">
            <input
              type="checkbox"
              name="has_daily_menu"
              checked={formData.has_daily_menu}
              onChange={handleChange}
              className="h-4 w-4 text-gray-900 focus:ring-primary border-gray-300 rounded"
            />
            <span className="ml-2 font-medium text-gray-700 dark:text-gray-200">Enable Daily Menu Updates</span>
          </label>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            When enabled, the system will send daily emails to request menu updates for specials and availability
          </p>
        </div>

        {formData.has_daily_menu && (
          <div>
            <label className="block font-medium text-gray-700 dark:text-gray-200">Daily Menu Update Email *</label>
            <input
              type="email"
              name="daily_menu_email"
              value={formData.daily_menu_email}
              onChange={handleChange}
              className={`mt-1 block w-full rounded-md border px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-primary/50 focus:outline-none ${
                errors.daily_menu_email ? "border-red-500" : "border-gray-300 dark:border-gray-600"
              }`}
            />
            {errors.daily_menu_email && <p className="mt-1 text-sm text-red-600">{errors.daily_menu_email}</p>}
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Daily menu update requests will be sent to this email address
            </p>
          </div>
        )}
      </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          onChange={handleChange}
          placeholder="e.g. Traditional Italian cuisine, molecular gastronomy, farm-to-table cooking..."
          className="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-primary/50 focus:outline-none"
        />
      </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Best Restaurant 2023"
                      className="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-primary/50 focus:outline-none"
                    />
                  </div>
                  <div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <span className="font-medium text-gray-700 dark:text-gray-300">Daily Menu Updates: </span>
            <span className="text-gray-900 dark:text-gray-100">{formData.has_daily_menu ? "Enabled" : "Disabled"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <p className="text-blue-800 dark:text-blue-200">
          Please review all the information above before submitting. Once submitted, your restaurant will be reviewed by our team before being published.
        </p>
      </div>
    </div>
  );
    ...
    <DashboardLayout>
      <LoadScript googleMapsApiKey={import.meta.env.VITE_GOOGLE_MAPS_API_KEY} libraries={libraries}>
        <div className="px-4 lg:px-8 xl:px-12">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Add New Restaurant</h1>
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              Complete all sections to add your restaurant to our platform
            </p>
          </div>
