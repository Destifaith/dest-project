...
const spaTreatments = ["Massage", "Facial", "Sauna", "Hydrotherapy", "Aromatherapy", "Body Scrub"];
const ambianceTypes = ["Relaxing", "Luxury", "Holistic", "Traditional"];
const facilitiesList = ["Private rooms", "Steam room", "Sauna", "Hot tubs", "Lounge area", "Refreshments", "Showers"];
...
const AddSpa = () => {
  const [formData, setFormData] = useState({
    ...
    opening_hours: {} as Record<string, { open: boolean; from: string; to: string }>,
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.main_image) newErrors.main_image = "Please select a main image.";
    ...
    data.append("treatment_type", formData.treatment_type);
    data.append("ambiance_type", formData.ambiance_type);
    ...
    router.post("/spas", data, {
      onSuccess: () => {
        ...
        setErrors({});
        setSuccessMessage("Spa added successfully!");
        if (mainImagePreview) {
        ...
        router.visit("/dashboard/entertainment/spa/manage");
      },
    ...
    <DashboardLayout>
      <LoadScript googleMapsApiKey={import.meta.env.VITE_GOOGLE_MAPS_API_KEY} libraries={libraries}>
        <div className="px-4 lg:px-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Add Spa</h1>
          <p className="mt-2 text-gray-700 dark:text-gray-300">Fill in the details below to add a new spa.</p>
          ...
          <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
            ...
            {/* Spa Name */}
            <div>
              <label className="block font-medium text-gray-700 dark:text-gray-200">Spa Name *</label>
              <input
                type="text"
                name="name"
                required
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="e.g. $50 per session"
                className="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-primary/50 focus:outline-none"
              />
            ...
            {/* Treatment & Ambiance Types */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block font-medium text-gray-700 dark:text-gray-200">Treatment Type *</label>
                <select
                  name="treatment_type"
                  required
                ...
                >
                  ...
                  {spaTreatments.map(type => (
                    <option key={type} value={type}>
                      {type}
                    </option>
                  ))}
                </select>
                {errors.treatment_type && <p className="text-red-500 text-sm mt-1">{errors.treatment_type}</p>}
              </div>
              <div>
                <label className="block font-medium text-gray-700 dark:text-gray-200">Ambiance Type *</label>
                <select
                  name="ambiance_type"
                  required
                  value={formData.ambiance_type}
                  onChange={handleChange}
                  className="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary/50 focus:outline-none"
                >
                  <option value="" disabled>
                    Select an ambiance type
                  </option>
                ...
                {errors.ambiance_type && <p className="text-red-500 text-sm mt-1">{errors.ambiance_type}</p>}
              </div>
            </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
              ...
              <div className="space-y-3">
                {daysOfWeek.map(day => (
                  <div
                    key={day}
                    className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-white dark:bg-gray-700 rounded-md"
                  >
              ...
              {galleryImagePreviews.length > 0 && (
                <div className="mt-3">
                  <h3 className="text-md font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Selected Images ({galleryImagePreviews.length})
                  </h3>
