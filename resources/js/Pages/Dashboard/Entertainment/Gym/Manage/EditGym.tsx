...
const EditGym: React.FC<Props> = ({ gym }) => {
  // Initialize gallery_images
  const initialGalleryImages = Array.isArray(gym.gallery_images)
    ? gym.gallery_images
    : typeof gym.gallery_images === 'string'
      ? JSON.parse(gym.gallery_images)
      : [];

  // Parse opening_hours if it's a string
  let parsedOpeningHours = gym.opening_hours;
  if (typeof gym.opening_hours === 'string') {
    try {
      parsedOpeningHours = JSON.parse(gym.opening_hours);
    } catch (e) {
  ...
  const parsedLatitude = parseFloat(gym.latitude?.toString()) || 0;
  const parsedLongitude = parseFloat(gym.longitude?.toString()) || 0;
  ...
  const [formData, setFormData] = useState({
    ...
    equipment_type: gym.equipment_type ? gym.equipment_type.split(',').map(v => v.trim()) : [],
    gym_type: gym.gym_type ? gym.gym_type.split(',').map(v => v.trim()) : [],
    facilities: gym.facilities ? gym.facilities.split(',').map(v => v.trim()) : [],
  ...
  const [previewMainImage, setPreviewMainImage] = useState<string | null>(gym.main_image ? `/storage/${gym.main_image}` : null);
  ...
  const gymTypeOptions: SelectOption[] = [
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    for (const [key, value] of Object.entries(formData)) {
      if (value !== null && value !== undefined) {
        ...
        } else if (['equipment_type', 'gym_type', 'facilities'].includes(key)) {
    ...
    router.post(`/gyms/${gym.id}?_method=PUT`, form, {
      forceFormData: true,
      onSuccess: () => {
        setErrors({});
      },
    ...
    <DashboardLayout>
      <LoadScript googleMapsApiKey={import.meta.env.VITE_GOOGLE_MAPS_API_KEY} libraries={libraries}>
        <div className="px-4 lg:px-6 py-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Gym</h1>
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Update the details below to edit the gym.
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-6">
            ...
            <div>
              ...
              <Select
                ...
                value={equipmentTypeOptions.filter(option => formData.equipment_type.includes(option.value))}
                onChange={(selected) => handleMultiSelectChange('equipment_type', selected)}
                className="mt-1"
              ...
              {errors.equipment_type && (
                <p className="mt-1 text-sm text-red-600 dark:text-red-400">{errors.equipment_type}</p>
              )}
            </div>
            ...
            <div>
              ...
              <Select
                ...
                value={gymTypeOptions.filter(option => formData.gym_type.includes(option.value))}
                onChange={(selected) => handleMultiSelectChange('gym_type', selected)}
                className="mt-1"
                classNamePrefix="select"
                styles={{
                  control: (base, state) => ({
              ...
              />
              {errors.gym_type && (
                <p className="mt-1 text-sm text-red-600 dark:text-red-400">{errors.gym_type}</p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="mt-1 block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-gray-800 hover:file:bg-primary/90"
              />
              {errors.main_image && (
            ...
            <div className="flex justify-end space-x-3">
              <Link
                href="/dashboard/entertainment/gym/manage"
              ...
              <button
                type="submit"
                className="px-4 py-2 bg-primary text-gray-800 rounded-md hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary/50"
              >
                Update Gym
              </button>
            </div>
